...
const ServiceHero = (servicetabProp) => {
  const [showCard, setShowCard] = useState(true);
  const { TabPane } = Tabs;
  
  const [servicetab, setservicetab] = useState(servicetabProp || []);
  const fetchdata = async () =>{
    try{
      const response = await axios.get('/api/servicetab' , servicetab)
      setservicetab(response.data.servicetab);
      console.log("API  tab:", response.data.servicetab);

    } catch(error){
      console.log(error, "Error getting service tab")
        ...
        <div className='block md:hidden shadow-lg rounded-md backdrop-blur-3xl w-full pb-20  p-3 h-full overflow-hidden overflow-y-scroll max-h-[84vh] sticky top-40 no-scrollbar'>
          ...
          <Anchor affix={false} onClick={closeCard} className={theme === 'dark' ? 'light-theme' : ' dark-theme'}>
            {
              tabshow.map((items , index)=>(
                <Link key={items.id || index + 1} href={items.href} title={items.title} />
              ))
            }
          </Anchor>
          <hr className='mt-5'/>
          <div className={` text-16 font-medium p-3 space-y-3`}>
          {
              tablink.map((items, index)=>(
                <NextLink key={items.id || index + 1} className={` ${
                  theme === 'dark' ? ' hover:text-primary-blue100' : 'hover:text-primary-blue100'
                } flex gap-2`} href={`${items.href}`}>
              {items.icon}
              {items.title}
            </NextLink>
              ))
...
<div className='hidden md:block shadow-lg rounded-md backdrop-blur-3xl w-full pb-20 md:w-full p-3 h-full overflow-hidden overflow-y-scroll max-h-[84vh] sticky top-40 no-scrollbar'>
        ...
        <div className={`p-1 md:p-3 space-y-10  `}>
          ...
          <Tabs defaultActiveKey="1"  tabBarStyle={{ border: 'none' }} className={``}>
            {
              Array.isArray(servicetab) &&
              servicetab.map((items , index)=>(
