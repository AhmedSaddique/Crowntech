  ...
  useEffect(() => {
    const fetchServiceInfo = async () => {
      try {
        ...
        console.log("Fetched service info:", response.data.serviceinfos);
      } catch (error) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-5 p-2">
        {serviceinfo.map((service, index) => (
          <div key={index} onClick={() => handleServiceClick(service.id)}
            className="bg-primary-white hover:bg-primary-light hover:scale-105 flex items-center transition duration-300 gap-2 p-3 shadow-lg text-primary-white rounded-md"
          >
            <Image
